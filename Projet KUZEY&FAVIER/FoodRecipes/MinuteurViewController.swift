...
class Minuteur: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
	...
	override func viewDidLoad() {
		...
		pickerInfo = ["5 min", "10 min", "15 min", "20 min", "30 min", "1 h"]
	...
	func compteur() {
		...
		if (!estActif) {
			...
			timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(Minuteur.incrementer), userInfo: nil, repeats: true)
			timer.fire()
